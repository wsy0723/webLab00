...
<!-- Ex. 2: Today’s Words & Ex 6: Query Parameters -->
        ...
        <ol>
            ...
            if(empty($_GET['number_of_words'])||!isset($_GET['number_of_words'])){
                $numberOfWords = 3;
            }else{
...
<!-- Ex. 3: Searching Words & Ex 6: Query Parameters -->
    ...
    <div class="section">
        ...
        if(empty($_GET['orderby'])||!isset($_GET['orderby'])){
            $orderby = '0';//판별할 수 있는 값이 들어오지 않았을 때
        }else if($_GET['orderby']=='1'){
            $orderby = '1';//1이 들어왔을 때 
        }else{
            $orderby='0';//1이외의 값이 들어왔을 때 
        }
        ?>
